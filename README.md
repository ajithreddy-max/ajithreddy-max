# Hi 👋, I'm Ajith Kumar BS  

🎓 Final-year Computer Science Engineering student  
💻 Aspiring Software Engineer | Web Developer  
🌱 Passionate about building real-world applications  

---

## 🚀 About Me  

- 👨‍🎓 Bachelor of Engineering (CSE) | CGPA: **8.7**  
- 💡 Interested in **🌐 Web Development, 💻 Software Engineer, 🎨 Frontend Developer, 🔐 Database Developer (SQL), ⚙️ Backend Developer**  
- 🔭 Worked on projects like **Doctor Appointment Booking App** and **AI-based Weather Forecasting System**  
- 🎯 Goal: To grow as a professional software engineer and contribute value to an organization  

---

## 🛠️ Technical Skills  

**Programming Languages:**  
- C  
- Python  
- Core Java  
- JavaScript

**Web Technologies**
- HTML
- CSS

**Developer Tools**  
- Git (Version Control)
- GitHub (Code Hosting & Collaboration)
- Visual Studio Code (Code Editor)
- Eclipse

**Developer Technologies**
- MongoDB (NoSQL Database)
- MySQL (Relational Database)
- REST APIs (Client-Server Communication)

**Platforms:**  
- Windows
- Ubuntu  

---

## 📂 Projects  

### 🩺 Doctor Appointment Booking App (MERN Stack)  
- Patient registration and doctor scheduling  
- Appointment booking system  
- Real-time availability management  

### 🌦️ AI-Powered Localized Weather Forecasting System  
- Frontend weather application using APIs  
- Displays temperature, rainfall, and wind data  
- Focused on agriculture-based forecasting  

### 🛒 E-Commerce Website for Grocery Marketing  
- Built using HTML, CSS, PHP, and MySQL  
- Features include cart, wishlist, and product filtering  
- Responsive and user-friendly design  

---

## 🏆 Experience & Activities  

- 🕒 Participated in **24-hour Buildathon (2025)** – CIT Gubbi  
- 💻 Developed multiple academic and personal projects  
- 🌐 Built responsive and API-based web applications  

---

## 📜 Certifications  

- 📘 Data Structures & Algorithms – Infosys Springboard  
- 🐍 Python 101 for Data Science – CognitiveClass (IBM)  

---

## 🌐 Languages  

- English  
- Telugu  
- Kannada  
- Hindi  

---

## 🎯 Hobbies  

- ✈️ Travelling  
- 🏏 Watching Sports & Movies  
- 🎮 Playing Games  

---

## 📫 Connect With Me  

- 📧 Email: **kumarajith19647@gmail.com**  
- 💼 LinkedIn: [linkedin.com/in/ajith-kumar-bs-a70859335](https://linkedin.com/in/ajith-kumar-bs-a70859335)  
- 🧑‍💻 GitHub: [github.com/ajithreddy-max](https://github.com/ajithreddy-max)
- Portfolio: [(https://ajithreddy-max.github.io/Portfolio/)

---

⭐ *Thank you for visiting my GitHub profile!*  
